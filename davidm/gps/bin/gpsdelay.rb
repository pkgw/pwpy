...
OP = OptionParser.new do |op|
  ...
  op.separator('')
  op.separator('Analyzes GPS time domain data')
  op.separator('')
  op.separator('PRN:')

  op.on('-p', '--prn=PRN', Integer, 'Specify PRN (required)') do |o|
    opts[:prn] = o
  end
  ...
  op.on('-c', '--chips=NCHIPS', Integer, 'Number of chips [40]') do |o|
    opts[:nchips] = o
  end
  op.on('-d', '--doppler=HZ', Float, 'Specify Doppler shift in Hz [0.0]') do |o|
    opts[:doppler] = o
  end
  ...
  op.on('-i', '--increment=STEP_CHIPS', Integer, 'Step increment in chips [NCHIPS/2]') do |o|
    opts[:step_size] = o
  end
  ...
  op.on('-o', '--offset=SKIP_CHIPS', Integer, 'Offset into data in chips [0]') do |o|
    opts[:offset] = o
  end
  op.on('-p', '--plotter=DEV', "PGPLOT device [ENV['PGPLOT_DEV']||'/xs']") do |o|
    opts[:device] = o
  end
  op.on('-s', '--steps=NSTEPS', Integer, 'Number of steps [20]') do |o|
    opts[:nsteps] = o
  end
...
if ARGV.empty? || opts[:prn].nil?
  puts OP.help()
  exit
end

# Default step size based on nchips
opts[:step_size] ||= opts[:nchips] / 2
