...
if ($help) {
	...
	print "\t--date [dd/mm/yyyy]\tspecify date of observation\n";
        print "\t--start [hh:mm:ss]\tspecify start time (local TZ)\n";
        print "\t--end [#]\tspecify end time in ATA decimal format (local TZ)\n";
...
if (!defined($STARTTIME)) {
	print "Enter start time (default 23:0:0): ";
	$STARTTIME=<STDIN>;
	chomp($STARTTIME);
}
$start = parse_string_time($STARTTIME);
if (!defined($ENDTIME)) {
	print "Enter end time (default 7): ";
	$ENDTIME=<STDIN>;
	chomp($ENDTIME);
}
...
print "Press return for graphical display: ";
<STDIN>;
system("cp .tmp/ds9.reg status/pigss-$DATE2.reg");
...
link "pigss.targets", "pigss.targets-$DATE2" || warn "Cannot link pigss.targets to pigss.targets-$DATE2\n";
print "Writing: pigss.targets-$DATE2\n";
