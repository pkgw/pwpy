...
#        for inp, preamble, data, flags, nread in uvdat.readAll ():
    ...
    def prep(self):
        self.data = (self.data * self.flags)[:,:][:,:,self.chans] 
        print 'Data flagged and trimmed in channels.'
...
#reorganize into dimensions of integration, baseline, channel
        ...
        abs = n.abs(data[:,noautos][:,:,chans].mean(axis=1))
...
#    p.axis([-0.5,len(chans)+0.5,100,0])
    ...
    def dmmask(self, dm = 60., t0 = 0., show=0):
        ...
        mask = n.zeros((self.data.shape[0],self.data.shape[2]),dtype=bool)   # could get clever here.  use integer code to stack dm masks in unique way
        ...
        if show:
            ax = p.imshow(mask, aspect='auto')
            p.colorbar(ax)

        return mask

    def dedisperse(self,da):
        pass
