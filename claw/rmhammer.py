...
cat = asciidata.AsciiData('/indirect/big_scr3/claw/data/ata/nvss-rm-best/rm-lb-lowres.txt')
rm = n.array(cat.columns[1])
l = n.array(cat.columns[3])
b = n.array(cat.columns[4])
...
for i in range(len(rm)):
    print l[i], b[i], rm[i]
    if rm[i] > 0.0:
        p.plot(n.radians([-1*l[i]]), n.radians([b[i]]), 'o', markersize=5+(abs(rm[i]/5.)), color='red', alpha=0.7)
    else:
        p.plot(n.radians([-1*l[i]]), n.radians([b[i]]), 'x', markersize=5+(abs(rm[i]/5.)), color='blue', alpha=1.0)
